...
2.  **Business Owner (Salon Manager)**
...
3.  **Staff Member (Service Provider)**
...
Based on pnpm workspaces \[15].

```text
/workspace-root
  /apps
    /frontend   (Next.js 14 App Router, PWA, Tailwind v4, shadcn/ui)
    /backend    (NestJS, Prisma ORM, PostgreSQL)
  /packages
    /shared     (TypeScript interfaces, Zod schemas, utilities)
  package.json
  pnpm-workspace.yaml
  docker-compose.yml   (Local dev: Nest, Next, Postgres)
```

### 5.2 Full Routing Map

**Client-Facing**
*   `/` - Home / Search
*   `/salons` - Directory
*   `/salons/[slug]` - Salon Profile
*   `/book/[businessId]` - Booking Start
*   `/book/[businessId]/service/[serviceId]` - Select Time
*   `/book/[businessId]/confirm` - Payment
*   `/auth/*` - Login/Register
*   `/client/dashboard` - User Profile

**Staff Portal**
*   `/staff/login`
*   `/staff/calendar`
*   `/staff/clients`
*   `/staff/client/[clientId]`
*   `/staff/account`
